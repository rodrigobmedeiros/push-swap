...
void    sort_small_stack(t_list **a_lst, t_list **b_lst)
{
    ...
    while (!(is_lst_sorted(*a_lst, 1) && init_size_a == ft_lstsize(*a_lst)))
    {
        if (apply_rotate(a_lst, NULL))
            continue ;
        ft_printf("passei rotate\n");
        if (apply_swap(a_lst, NULL))
            continue ;
        ft_printf("passei swap\n");
        if (apply_reverse_rotate(a_lst, NULL))
            continue ;
        ft_printf("passei reverse rotate\n");
        //if (apply_push(a_lst, NULL))
        //    continue ;
...
int find_small_number(t_list *lst)
{
    ...
    while (lst)
    {
        if (get_node_content(lst) < min_value)
            min_value = get_node_content(lst);
        lst = lst->next;
    }
    return (min_value);
}

void    apply_n_rotate(t_list **lst, int n, enum OpName op)
{
    int count;
...
void    apply_rotations_if_needed(t_list **lst, int min_value)
{
    ...
    {
        min_position++;
        if (min_value == get_node_content(temp))
            break;
        temp = temp->next;
    }
